...
<div class="page-container">
	<div class="text-c">
		<button onclick="queryusedata(1)" class="btn btn-primary radius">显示正常科室</button>
		<button onclick="querystopdata(1)" class="btn btn-primary radius">显示停用科室</button>
		</span> 日期范围：
		<input type="text" onfocus="WdatePicker({ maxDate:'#F{$dp.$D(\'logmax\')||\'%y-%M-%d\'}' })" id="logmin" class="input-text Wdate" style="width:120px;">
		-
		<input type="text" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'logmin\')}',maxDate:'%y-%M-%d' })" id="logmax" class="input-text Wdate" style="width:120px;">
		<input type="text" name="" id="searchindex" placeholder=" 科室名称" style="width:250px" class="input-text">
		<button name="" id="" class="btn btn-success"  onclick="querysomedata(1,1)" type="submit"><i class="Hui-iconfont">&#xe665;</i> 搜科室</button>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="departments_add('添加科室','${pageContext.request.contextPath}/departments/departmentsupdate.do','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加科室</a></span>
...
*/
/*科室-增加*/
function departments_add(title,url,w,h){
	layer_show(title,url,w,h);
}


//科室-编辑
function departments_edit(title,url,id,w,h){
...
//科室-删除
function departments_del(obj,id){
    swal({
        title: "",      //弹出框的title
		...
		$.ajax({
            type: "post",
            url: "${pageContext.request.contextPath}/departments/deldepartments.action",
            dataType: 'json',
...
function datadel(){
    ...
    swal({
		...
		$.ajax({
            type: "post",
            url: "${pageContext.request.contextPath}/departments/delalldepartments.action",
            dataType: 'json',
            contentType: "application/json;charset=utf-8",
...
function querysomedata(currpage,delflags){
	...
	var searchindex=$("#searchindex").val();
		$.ajax({
           type: "post",
           url: "${pageContext.request.contextPath}/departments/querysomedata.action?date1="
           ...
           success: function (page) {
               ...
               if(totalsize!=0){
               	...
               	for(var i=0;i<data1.length;i++){
	               		...
	               		var hang3=$('<td>'+data1[i].departmentsName+'</td>');
	               		var hang4=$('<td>'+data1[i].departmentsNum+'</td>');
	               		var hang41=$('<td>'+data1[i].departmentsAddress+'</td>');
	               		var hang5=$('<td>'+data1[i].departmentsContent+'</td>');
	               		var hang6=$('<td>'+data1[i].departmentsRemarks+'</td>');
	               		var hang7=$('<td>'+data1[i].departmentsDate+'</td>');
	               		var hang8=$('<td>'+state+'</td>');
	               		var hang9=$('<td class="f-14 td-manage"><a style="text-decoration:none" class="ml-5" onClick="departments_edit(\'科室编辑\',\'${pageContext.request.contextPath}/departments/departmentsupdate.do?id='+data1[i].id+'\',\''+data1[i].id+'\',\'800\',\'500\')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="ml-5" onClick="departments_del(this,\''+data1[i].id+'\')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>');
	               		$(lie).append(hang1);
	               		$(lie).append(hang2);
	               		$(lie).append(hang3);
	               		$(lie).append(hang4);
