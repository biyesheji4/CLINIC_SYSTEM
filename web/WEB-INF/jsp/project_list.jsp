...
<div class="page-container">
	<div class="text-c">
		<button onclick="queryusedata(1)" class="btn btn-primary radius">显示正常项目</button>
		<button onclick="querystopdata(1)" class="btn btn-primary radius">显示停用项目</button>
		</span> 日期范围：
		<input type="text" onfocus="WdatePicker({ maxDate:'#F{$dp.$D(\'logmax\')||\'%y-%M-%d\'}' })" id="logmin" class="input-text Wdate" style="width:120px;">
		-
		<input type="text" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'logmin\')}',maxDate:'%y-%M-%d' })" id="logmax" class="input-text Wdate" style="width:120px;">
		<input type="text" name="" id="searchindex" placeholder=" 项目名称" style="width:250px" class="input-text">
		<button name="" id="" class="btn btn-success" onclick="querysomedata(1,1)" type="submit"><i class="Hui-iconfont">&#xe665;</i> 搜项目</button>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="projects_add('添加项目','${pageContext.request.contextPath}/project/projectupdate.do','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加项目</a></span>
...
*/
/*项目-增加*/
function projects_add(title,url,w,h){
...
function project_del(obj,id){
    swal({
		...
		$.ajax({
            type: "post",
            url: "${pageContext.request.contextPath}/project/delproject.action",
            dataType: 'json',
...
function querysomedata(currpage,delflags){
	...
	var searchindex=$("#searchindex").val();
		$.ajax({
           type: "post",
           url: "${pageContext.request.contextPath}/project/querysomedata.action?date1="
           ...
           success: function (page) {
               ...
               if(totalsize!=0){
               	...
               	for(var i=0;i<data1.length;i++){
	               		...
	               		var hang3=$('<td>'+data1[i].projectsName+'</td>');
	               		var hang4=$('<td>'+data1[i].projectsNum+'</td>');
	               		var hang5=$('<td>'+data1[i].projectsContent+'</td>');
	               		var hang6=$('<td>'+data1[i].projectsRemarks+'</td>');
	               		var hang52=$('<td>'+data1[i].projectMoney+'</td>');
	               		var hang7=$('<td>'+data1[i].projectsDate+'</td>');
	               		var hang8=$('<td>'+state+'</td>');
	               		var hang9=$('<td class="f-14 td-manage"><a style="text-decoration:none" class="ml-5" onClick="project_edit(\'项目编辑\',\'${pageContext.request.contextPath}/project/projectupdate.do?id='+data1[i].id+'\',\''+data1[i].id+'\',\'800\',\'500\')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="ml-5" onClick="project_del(this,\''+data1[i].id+'\')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>');
	               		$(lie).append(hang1);
	               		$(lie).append(hang2);
	               		$(lie).append(hang3);
	               		$(lie).append(hang4);
